...
.PARAMETER agent
    This is the agent name for puppet

.PARAMETER Msi
    This is the agent msi file

.PARAMETER MsiUrl
    This is the URL to the Puppet MSI file you want to install. This defaults
    to a version from PuppetLabs.
#>
Param([string] $agent, [string]$Msi = "puppet-3.7.3.msi", [string]$MsiUrl = "http://puppetlabs.com/downloads/windows/")
...
$downloadPath = "C:\vagrant\$Msi"
$webClient = New-Object System.Net.WebClient
$webClient.DownloadFile($MsiUrl, $downloadPath)

# Install it
Write-Host "Installing Puppet..."
$install_args = @("/qn", "/i", $downloadPath, "PUPPET_MASTER_SERVER=puppet.nacswildcats.dev", "PUPPET_AGENT_CERTNAME=$agent.nas.dev")
